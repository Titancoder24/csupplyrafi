...
export default function ProductsScreen() {
  ...
  async function fetchProducts() {
    ...
    const { data, error } = await supabase
      .from('products')
      .select('id, name, base_price, unit, status, created_at, images, vendor_id, categories(name), vendor_profiles(shop_name)')
      .eq('status', tab)
      .is('deleted_at', null)
    ...
    if (!error) {
      setProducts((data ?? []).map((d: any) => ({
        ...
        vendor_name: d.vendor_profiles?.shop_name ?? `Vendor (${d.vendor_id?.slice(0, 8)})`,
      })));
    }
    setLoading(false);
  }
                  ...
                  <View style={s.actionRow}>
                    ...
                    >
                      {acting === p.id
                        ...
                        : <><CheckCircle2 size={14} color="#fff" strokeWidth={2.5} /><Text style={s.actionText}>Approve</Text></>}
                    </Pressable>
                    <Pressable
                      ...
                      disabled={acting === p.id}
                    >
                      <XCircle size={14} color="#fff" strokeWidth={2.5} />
