      ...
      {[1, 2, 3, 4, 5].map(n => (
        <Pressable key={n} onPress={() => onChange(n)} hitSlop={6}>
          <Star
            size={36}
            color="#0F172A"
            fill={n <= value ? '#0F172A' : 'transparent'}
            strokeWidth={1.5}
          />
        </Pressable>
      ))}
...
export default function OrderDetailScreen() {
  ...
  const load = useCallback(async (silent = false) => {
    ...
    const { data } = await supabase
      .from('orders')
      .select('id, order_number, status, total_amount, created_at, transporter_id, addresses!orders_delivery_address_id_fkey(line1, city, state), order_items(id, product_name, quantity, unit, unit_price, line_total, products(images))')
  ...
  useEffect(() => {
    ...
    const ch = supabase.channel(`order-detail-rt:${id}:${suffix}`)
  ...
  if (!order) return (
    <View style={{ flex: 1, backgroundColor: BG, alignItems: 'center', justifyContent: 'center', gap: 12 }}>
      <Package size={48} color="#0F172A" strokeWidth={1.4} />
      ...
      >
        ...
        {isRejected ? (
          <View style={[s.banner, { backgroundColor: '#FEF2F2', borderColor: '#FECACA' }]}>
            <AlertCircle size={20} color="#0F172A" strokeWidth={2} />
            <View style={{ flex: 1 }}>
        ...
        {/* Truck animation — shows from transporter accept all the way through delivery */}
        {[
          ...
          <TruckAnimation status={order.status} />
        )}
            ...
            <View style={s.transporterHeader}>
              ...
              {order.transporter.phone && (
                <View style={s.callBtn}>
                  <Phone size={14} color="#0F172A" strokeWidth={2} />
          ...
          <View style={s.card}>
            {ratingDone ? (
              <View style={{ alignItems: 'center', gap: 8, paddingVertical: 8 }}>
                <CheckCircle2 size={32} color="#0F172A" strokeWidth={2} />
                ...
                >
                  <View style={s.thumbWrap}>
                    <Image
                      source={{
                        uri: getProductImage(
                          { name: item.product_name, images: (item as any).products?.images },
                          { size: 200 },
                        ),
                      }}
                      style={s.thumb}
        ...
        {order.delivery_address && (
          <View style={s.card}>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'flex-start', gap: 10 }}>
              <MapPin size={16} color="#0F172A" strokeWidth={2} style={{ marginTop: 1 }} />
          ...
          <Pressable style={rm.card} onPress={() => {}}>
            {ratingDone ? (
              ...
              <ScrollView showsVerticalScrollIndicator={false}>
                {/* Header — biker icon on left, title middle, close + product thumbnail right */}
...
const rm = StyleSheet.create({
  ...
  closeTxt:  { fontSize: 26, color: '#94A3B8', lineHeight: 28, fontWeight: '300' as const },
