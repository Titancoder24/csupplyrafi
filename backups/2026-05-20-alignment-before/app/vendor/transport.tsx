    ...
    <View style={mf.root}>
      ...
      {value && value.lat !== 0 && (
        <View style={mf.chip}>
          ...
          <Text style={[mf.chipTxt, { color: dotColor }]} numberOfLines={1}>{value.address}</Text>
        </View>
      )}
    </View>
  );
...
const mf = StyleSheet.create({
  ...
  chip: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: '#F0FDF4', borderRadius: 8, paddingHorizontal: 10, paddingVertical: 6, borderWidth: 1, borderColor: '#BBF7D0' },
...
function IncreaseOfferPanel({ request, onIncrease, loading }: {
  ...
  return (
    <View style={ip.root}>
      <Text style={ip.title}>No transporter accepted — increase your offer and re-submit for admin approval</Text>
      ...
      >
        {loading
          ...
          : <Text style={ip.btnTxt}>Re-submit with {fmtINR(newPrice)} offer</Text>}
      </Pressable>
    </View>
  );
}
...
const ip = StyleSheet.create({
  ...
  bumpBtnSel: { borderColor: ACCENT, backgroundColor: '#F0FDF4' },
...
export default function VendorTransportScreen() {
  ...
  const [dropoff, setDropoff] = useState<{ address: string; lat: number; lng: number } | null>(() => {
    if (params.prefill_dropoff) {
      return {
        address: params.prefill_dropoff,
        lat: parseFloat(params.prefill_lat ?? '0') || 0,
        lng: parseFloat(params.prefill_lng ?? '0') || 0,
      };
    }
    return null;
  });
  const [description, setDesc]  = useState(
    params.prefill_order_number ? `Delivery for Order #${params.prefill_order_number}` : ''
  );
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`vendor-transport-rt:${user.id}`)
      .on('postgres_changes', {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const { data: inserted, error: dbErr } = await supabase
        .from('transport_requests')
        .insert({
          requester_id: uid, requester_role: 'vendor',
          ...
          order_id: params.prefill_order_id || null,
        })
        .select('id, pickup_address, dropoff_address, distance_km, weight_kg, vehicle_class, status, offered_price, created_at')
        .single();
  ...
  return (
    <SafeAreaView edges={['top']} style={{ flex: 1, backgroundColor: ACCENTH }}>
      {/* Header */}
      <View style={s.header}>
        ...
        <View style={s.headerCenter}>
          ...
          <Text style={s.headerSub}>Vendor Portal</Text>
        </View>
        <View style={{ width: 40 }} />
      </View>
...
const s = StyleSheet.create({
  header: { backgroundColor: ACCENTH, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingBottom: 16, paddingTop: 6, gap: 12 },
  ...
  headerSub:   { fontFamily: FontFamily.regular, fontSize: 11, color: 'rgba(255,255,255,0.65)' },

  tabWrap: { backgroundColor: ACCENTH, paddingHorizontal: 20, paddingBottom: 16 },
  tabBar: { flexDirection: 'row', backgroundColor: 'rgba(255,255,255,0.15)', borderRadius: 12, padding: 3 },
  tab: { flex: 1, paddingVertical: 9, alignItems: 'center', borderRadius: 10 },
  tabActive: { backgroundColor: '#fff' },
  tabTxt: { fontFamily: FontFamily.semiBold, fontSize: 13, color: 'rgba(255,255,255,0.7)' },
  tabTxtActive: { color: ACCENTH },
  ...
  vehicleCardSel: { backgroundColor: '#F0FDF4', borderColor: ACCENT },
