...
import { RechartsLine, type LinePoint } from '@/components/ui/charts/RechartsLine';
import { RechartsDonut } from '@/components/ui/charts/RechartsDonut';
    ...
    <Pressable style={s.kpiTile} onPress={onPress}>
      <View style={s.kpiHead}>
        <View style={[s.kpiIcon, { backgroundColor: accentBg }]}>
          <Icon size={14} color="#0F172A" strokeWidth={1.75} />
        </View>
      ...
      <View style={s.kpiFoot}>
        {trend ? (
          <View style={s.kpiTrendRow}>
            {trend.positive
              ? <TrendingUp   size={12} color="#0F172A" strokeWidth={1.75} />
              : <TrendingDown size={12} color="#0F172A"   strokeWidth={1.75} />}
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={[s.chartsRow, isNarrow && { flexDirection: 'column' }]}>
          ...
          <View style={[s.chartCard, isNarrow ? { width: '100%' } : { flex: 1.6 }]}>
            <View style={s.chartHead}>
              ...
              <Pressable style={s.rangePill} onPress={() => setRangeOpen(true)}>
                ...
                <ChevronDown size={12} color="#0F172A" strokeWidth={1.75} />
              </Pressable>
            </View>
            {ordersByDay.length > 0 ? (
              <RechartsLine data={ordersByDay} height={210} color={C.text} />
            ) : (
          ...
          <View style={[s.chartCard, isNarrow ? { width: '100%' } : { flex: 1 }]}>
            ...
            {totalStatus > 0 ? (
              <RechartsDonut
                data={DONUT.map(d => ({ label: d.label, value: d.value }))}
                size={160}
              ...
              return (
                <View key={u.id} style={[s.tRow, i === recentUsers.length - 1 && { borderBottomWidth: 0 }]}>
                  ...
                  <Pressable style={{ width: 30, alignItems: 'center' }} hitSlop={4}>
                    <MoreVertical size={14} color="#0F172A" strokeWidth={1.75} />
                  </Pressable>
                </View>
              );
            })
            ...
            {RANGE_OPTIONS.map(opt => {
              ...
              return (
                ...
                >
                  ...
                  {selected ? <Check size={14} color="#0F172A" strokeWidth={2.4} /> : <View style={{ width: 14 }} />}
                </Pressable>
              );
            })}
          </Pressable>
