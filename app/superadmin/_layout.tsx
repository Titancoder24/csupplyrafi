...
import {
  ...
  CheckCheck, Star, CheckCircle2, LifeBuoy,
...
const NAV_SECTIONS = [
  ...
  },
  {
    ...
    items: [
      ...
      { key: 'support',              label: 'Support Tickets',     icon: LifeBuoy,     route: '/superadmin/support' },
  ...
  return (
    <View style={[sb.root, collapsed && sb.collapsed]}>
      ...
      <View style={sb.logoRow}>
        ...
        <Pressable onPress={onToggle} hitSlop={8} style={sb.collapseBtn}>
          {collapsed
            ...
            : <ChevronLeft  size={15} color="#0F172A" strokeWidth={2} />
          }
        </Pressable>
      </View>
      ...
      <View style={sb.footer}>
        ...
        >
          <LogOut size={16} color="#0F172A" strokeWidth={1.8} />
  ...
  return (
    <View style={np.panel}>
        ...
        <View style={{ flexDirection: 'row', gap: 12, alignItems: 'center' }}>
          {unreadCount > 0 && (
            <Pressable onPress={markAllRead} style={np.markAllBtn}>
              <CheckCheck size={13} color="#0F172A" strokeWidth={2} />
      ...
      {notifications.length === 0 ? (
        <View style={np.empty}>
          <Bell size={28} color="#0F172A" strokeWidth={1.5} />
    ...
    <View style={tb.root}>
      ...
      <View style={tb.right}>
        ...
        <Pressable style={tb.iconBtn} onPress={() => setShowNotifs(true)}>
          <Bell size={18} color="#0F172A" strokeWidth={1.75} />
          {unreadCount > 0 && (
            <View style={tb.badge}>
...
export default function SuperAdminLayout() {
  ...
  // Track the last value so we only call setState on an actual transition.
  const lastNarrowRef = React.useRef<boolean | null>(null);
  useEffect(() => {
    if (lastNarrowRef.current === isNarrow) return;
...
function ActionPopupQueue() {
  ...
  if (current.kind === 'new_order') {
    ...
    return (
      <ActionPopup
        visible
        icon={<ShoppingCart size={28} color="#0F172A" strokeWidth={2.2} />}
        iconBg="#EFF6FF"
  ...
  if (current.kind === 'pickup') {
    return (
      <ActionPopup
        visible
        icon={<Package size={28} color="#0F172A" strokeWidth={2.2} />}
        iconBg="#FFFBEB"
  ...
  if (current.kind === 'delivery') {
    return (
      <ActionPopup
        visible
        icon={<CheckCircle2 size={28} color="#0F172A" strokeWidth={2.2} />}
        iconBg="#DCFCE7"
...
const tb = StyleSheet.create({
  ...
  badge: {
    position: 'absolute' as never, top: 4, right: 4,
  ...
  badgeText: { fontFamily: FontFamily.semiBold, fontSize: 8, color: '#fff', lineHeight: 10 },
  vDivider: { width: 1, height: 20, backgroundColor: T.border, marginHorizontal: 6 },
  ...
  avatarText: { fontFamily: FontFamily.semiBold, fontSize: 11, color: '#fff', letterSpacing: 0.2 },
  avatarName: { fontFamily: FontFamily.semiBold, fontSize: 13, color: T.textPrimary, letterSpacing: -0.05 },
  avatarRole: { fontFamily: FontFamily.regular, fontSize: 11, color: T.textMuted },
});
