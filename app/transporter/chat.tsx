...
import { ArrowLeft, Send, User, CheckCheck } from 'lucide-react-native';
import { SafeAreaView } from 'react-native-safe-area-context';
import { supabase } from '@/services/supabase';
import { useAuth } from '@/services/auth/AuthProvider';

const PRIMARY = '#1A5C30', BG = '#F8FAFC', SURF = '#FFFFFF';
...
export default function TransporterChatScreen() {
  ...
  useEffect(() => {
    ...
    if (!orderId) return;
    const ch = supabase
      .channel(`transporter-chat-rt:${orderId}`)
      .on(
        'postgres_changes',
  ...
  const renderItem = ({ item }: { item: Message }) => {
    ...
    return (
      <View style={[ms.bubble, isMe ? ms.bubbleMe : ms.bubbleThem]}>
        {!isMe && (
          <Text style={ms.senderLabel}>{customerName ?? 'Customer'}</Text>
        )}
        ...
        <View style={ms.timeRow}>
          ...
          {isMe && <CheckCheck size={12} color="#FFFFFF" strokeWidth={2} />}
        </View>
      </View>
    );
  };
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: PRIMARY }} edges={['top']}>
      ...
      <View style={s.header}>
        ...
        <View style={s.customerInfo}>
          ...
          <View style={{ gap: 1 }}>
            <Text style={s.customerName}>{customerName ?? 'Customer'}</Text>
            <Text style={s.customerStatus}>Order #{orderId?.slice(-6).toUpperCase()}</Text>
          </View>
        </View>
      </View>
      ...
      >
            ...
            ListEmptyComponent={
              <View style={s.emptyChat}>
                <View style={s.emptyChatIcon}>
                  <User size={32} color="#0F172A" strokeWidth={1.5} />
                </View>
                <Text style={s.emptyChatTitle}>No messages yet</Text>
                <Text style={s.emptyChatSub}>
                  Send a quick update to the customer about your delivery status
                </Text>
              </View>
            }
        ...
        <View style={s.quickBar}>
          <FlatList
            ...
            renderItem={({ item }) => (
              <Pressable
                style={({ pressed }) => [s.quickChip, pressed && { opacity: 0.75 }]}
...
const ms = StyleSheet.create({
  ...
  time:       { fontFamily: R_REG, fontSize: 10, color: MUTED },
});
...
const s = StyleSheet.create({
  ...
  customerInfo:   { flex: 1, flexDirection: 'row', alignItems: 'center', gap: 10 },
  customerAvatar: {
    width: 40, height: 40, borderRadius: 20,
    backgroundColor: 'rgba(255,255,255,0.2)', alignItems: 'center', justifyContent: 'center',
  },
  customerName:   { fontFamily: R_BOLD, fontSize: 15, color: '#fff' },
  customerStatus: { fontFamily: R_REG, fontSize: 11, color: 'rgba(255,255,255,0.65)' },
  ...
  quickBar: {
    paddingVertical: 10, borderTopWidth: 1, borderTopColor: BORDER, backgroundColor: SURF,
  },
  quickChip: {
    paddingHorizontal: 14, paddingVertical: 8, borderRadius: 20,
    backgroundColor: '#EFF6FF', borderWidth: 1, borderColor: '#BFDBFE',
  },
  quickChipTxt: { fontFamily: R_MED, fontSize: 13, color: '#1D4ED8' },
